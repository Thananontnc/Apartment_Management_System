    ...
    return (
        <main className="container animate-fade-in" style={{ paddingBottom: '8rem' }}>
            <div className="no-print" style={{ display: 'flex', justifyContent: 'flex-start', paddingTop: '24px' }}>
                <BackButton label={<TranslatedText tKey="asset_portfolio" />} href={`/apartments/${apartment.id}`} />
            </div>

            <header className="no-print" style={{ padding: '40px 0 60px 0' }}>
                <div className="flex-between flex-wrap gap-24">
                    <div>
                        <h1 className="text-gradient">{t('generated_invoices')}</h1>
                        <p style={{ color: 'var(--text-muted)', fontSize: '1.2rem', fontWeight: '500', marginTop: '8px' }}>{t('asset_portfolio')}: {apartment.name}</p>
                    </div>
                    <div className="no-print" style={{ display: 'flex', gap: '16px', alignItems: 'center', flexWrap: 'wrap' }}>
                        {months.length > 0 && (
                            ...
                            >
                                <option value="">{t('latest_records')}</option>
                                {months.map(m => (
                                    <option key={m.value} value={m.value}>{formatMonth(m.value)}</option>
                                ))}
                            </select>
                        )}
                        <button
                        ...
                        >
                            <span style={{ marginRight: '8px' }}>🖨️</span> {t('print_ledger')}
                        </button>
                    </div>
                </div>
            </header>
                    ...
                    return (
                        <div key={room.id} className="invoice-card" style={{
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '3px solid #1a1a1a', paddingBottom: '24px', marginBottom: '24px' }}>
                                <div>
                                    <b style={{ color: '#888', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '1px' }}>{t('unit_designation')}</b>
                                    <div style={{ fontSize: '32px', fontWeight: '950', color: '#1a1a1a' }}>{room.roomNumber}</div>
                                </div>
                                <div style={{ textAlign: 'right' }}>
                                    <b style={{ color: '#888', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '1px' }}>{t('statement_date')}</b>
                                    <div style={{ fontSize: '16px', fontWeight: '700', color: '#1a1a1a' }}>{formatDate(new Date(bill.recordDate))}</div>
                            ...
                            <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '8px' }}>
                                <thead>
                                    <tr style={{ borderBottom: '1px solid #eee' }}>
                                        <th style={{ textAlign: 'left', padding: '12px 0', fontSize: '11px', color: '#888', textTransform: 'uppercase', letterSpacing: '1px' }}>{t('qty')}</th>
                                        <th style={{ textAlign: 'left', padding: '12px 0', fontSize: '11px', color: '#888', textTransform: 'uppercase', letterSpacing: '1px' }}>{t('item_description')}</th>
                                        <th style={{ textAlign: 'right', padding: '12px 0', fontSize: '11px', color: '#888', textTransform: 'uppercase', letterSpacing: '1px' }}>{t('rate')}</th>
                                        <th style={{ textAlign: 'right', padding: '12px 0', fontSize: '11px', color: '#888', textTransform: 'uppercase', letterSpacing: '1px' }}>{t('subtotal')} (฿)</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr style={{ borderBottom: '1px solid #f9f9f9' }}>
                                        <td style={{ padding: '16px 0', fontWeight: '700' }}>{bill.waterUsage.toFixed(0)}</td>
                                        <td style={{ padding: '16px 0' }}>{t('water_usage_bill')}</td>
                                    ...
                                    <tr style={{ borderBottom: '1px solid #f9f9f9' }}>
                                        ...
                                        <td style={{ padding: '16px 0' }}>{t('elec_usage_bill')}</td>
                                    ...
                                    <tr style={{ borderBottom: '1px solid #f9f9f9' }}>
                                        ...
                                        <td style={{ padding: '16px 0' }}>{t('base_rent_bill')}</td>
                            ...
                            <div style={{ marginTop: 'auto', background: '#fcfcfc', border: '1px solid #f0f0f0', padding: '20px', borderRadius: '16px' }}>
                                <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr', textAlign: 'center', marginBottom: '12px', fontSize: '10px', color: '#999', fontWeight: '800', textTransform: 'uppercase', letterSpacing: '1px' }}>
                                    <div style={{ textAlign: 'left' }}>{t('metrics_audit')}</div>
                                    <div>{t('present')}</div>
                                    <div>{t('previous')}</div>
                                </div>
                                <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr', textAlign: 'center', marginBottom: '8px' }}>
                                    <div style={{ textAlign: 'left', fontWeight: '700', fontSize: '13px' }}>{t('water_meter')}</div>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr', textAlign: 'center' }}>
                                    <div style={{ textAlign: 'left', fontWeight: '700', fontSize: '13px' }}>{t('energy_meter')}</div>
                            ...
                            <div style={{
                                ...
                                <span style={{ fontSize: '12px', fontWeight: '900', textTransform: 'uppercase', letterSpacing: '2px', color: '#1a1a1a' }}>{t('grand_total_due')}</span>
                                <span style={{ fontWeight: '950', fontSize: '32px', color: '#1a1a1a' }}>
                                    {t('bath_unit')}{bill.totalAmount.toLocaleString()}
                                </span>
                            </div>
                        </div>
                    );
